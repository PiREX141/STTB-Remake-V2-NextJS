...
export async function createLecturer(data: CreateLecturerRequest, token: string): Promise<CreateLecturerResponse> {
    const response = await fetch(`${API_BASE_URL}/api/Dosen`, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
        ...
        throw new Error(error?.message ?? `Failed to create lecturer: ${response.status}`);
    }
