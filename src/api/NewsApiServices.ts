...
export async function createNews(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error?.message ?? `Failed to create news: ${response.status}`,
    );
  }
...
export async function updateNews(
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(
      error?.message ?? `Failed to update news: ${response.status}`,
    );
  }
...
export async function deleteNews(
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(
      error?.message ?? `Failed to delete news: ${response.status}`,
    );
  }
