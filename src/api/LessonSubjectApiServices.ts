...
export async function createLessonSubject(data: CreateLessonSubjectRequest, token: string): Promise<CreateLessonSubjectResponse> {
    const response = await fetch(`${API_BASE_URL}/api/MataKuliah`, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
        ...
        throw new Error(error?.message ?? `Failed to create lesson subject: ${response.status}`);
    }
