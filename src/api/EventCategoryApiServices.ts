...
export async function createEventCategory(data: CreateEventCategoryRequest): Promise<EventCategory> {
    const response = await fetch(`${API_BASE_URL}/api/KategoriKegiatan`, {
        method: "POST",
    ...
    if (!response.ok) {
        ...
        throw new Error(error?.message ?? `Failed to create event category: ${response.status}`);
    }
